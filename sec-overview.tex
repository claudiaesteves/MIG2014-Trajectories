...
\begin{figure}[th]
	\begin{center}
	\includegraphics[width=0.9\linewidth]{./images/patch-definitions-arrows-x.png}
	\caption{
...
These last two sets may be empty in the case of an empty patch.
...
\begin{enumerate}
	...
	\item If the sequence is composed by more than one trajectories, the ending position of one trajectory should be the same as the starting position of the next one to ensure continuity\footnote{Please refer to~\cite{Yersin:2009} for more details.}:
...
% 
% \item[Pattern -]{
A patch can be considered as a spatio-temporal right prism depending on the type of polygon used for its area $\mathbf{A}$ (cube in the case of a square patch).
...
When these patches are animated, agents will be moving from one patch to its adjacent ones. 
...
\begin{figure*}[ht]
	\begin{center}
	\includegraphics[width=0.9\linewidth]{./images/overview-hd-x.png}
...
Intuitively, there are some matches that are better than others; e.g., judging by observation, trajectories passing near the center of the patch look better than the ones staying close to the borders.
...
%
Algorithm~\ref{alg:stable-matching} demonstrates the Gale-Shapley algorithm in relation to two equal lists of entry and exit points  who are being matched for pairing.
...
Initialize all $i \in \mathbf{I}$ and $o \in \mathbf{O}$ to \textit{free} \;
\While {$ \mathbf{\exists}$ \textit{free} entry point $i$ who still has an exit point $o$ to propose to} {
	...
	}
	{
		...
		\eIf {$o$ prefers $i$ to $i'$} 
		{
			$(i,o)$ become paired \;
			$i'$ becomes \textit{free} \;
		}
		{
			 $(i',o)$ remain paired;
		}
...
To do so, all entry and exit points keep a \emph{proposal list} $\mathbf{L}_s$ indicating the order of preference for their matching (Table~\ref{tab:proposal-list}).
...
\begin{table}[t]
	...
	Smaller values indicate higher preference, with exit point B in this case being the most preferred one.
	Exit Points F and D lie in the same pattern as Entry Point $1$, so they receive a larger score.
	}
	\begin{tabular}{|c|c|}
...
It should be emphasized here, that all control points (both entry and exit) keep their own proposal lists.
...
every two points that remain engaged at the end of the algorithm become a pair.
...
\begin{figure*}[t]
	\centering
	\includegraphics[width=0.5\linewidth]{./images/key-entry-exit-point-hor-x.png}
	\\	
	\begin{subfigure}[b]{0.27\linewidth}
		\includegraphics[width=\linewidth]{./images/collision-2D-example-with-collision-x.png}
		\caption{}
	\end{subfigure}
	~~~~~% 
	\begin{subfigure}[b]{0.27\linewidth}
		\includegraphics[width=\linewidth]{./images/collision-2D-example-without-collision-x.png}
		\caption{}
	\end{subfigure}
	~~~~~%
	\begin{subfigure}[b]{0.27\linewidth}
		\includegraphics[width=\linewidth]{./images/collision-2D-example-overlay-x.png}
		\caption{}
	\end{subfigure}
	\caption{
...
Since patches are concatenated together to create larger crowds, care \textit{must} be taken during trajectory modification so that the spatio-temporal boundary control points (i.e., entry and exit ones) are not modified; other control points can be added and manipulated.
...
and finally $w_i$ is a weight to reduce speed artifacts and prevent agents from leaving the bounds of the patch:
...
Once $\mathbf{p}_i^{new}$ is found, a check to find if there is an existing control point within a small time interval is performed; if successful, then that point is moved to $\mathbf{p}_i^{new}$, otherwise a new control point is added at $\mathbf{p}_i^{new}$.
...
However there are still situations where it converges slowly or even gets stuck in an infinite loop; and therefore a maximum number of iterations is set.
...
\textbf{Minimum Distance} The minimum distance between two trajectories is defined as their minimum spatio-temporal distance; i.e. at 	the point in time where they are closest to each other.
...
For each one of the trajectory's segments, the spline's coefficients need to be found under $C^1$ continuity restrictions:
...
There may be cases that even using a large number of tacks the splines will differ significantly from the original ones. 
% that the splines still vary too much from the original trajectory.
...
%We define a threshold based on the same threshold of collision to know how far a way a point can be moved.
