...
\While{there exists at least one entry in $M$ below the threshold}{
	Find indexes $i$  and $j$ for which $M(i,j)$ has the smallest value $d$\;
	Find temporal control points $\mathbf{cp}_i$ and $\mathbf{cp}_j$ in $\tau_i$ and $\tau_j$ that are at distance $d$ \;
