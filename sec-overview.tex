...
We compute a minimum distance between each pair of trajectories. We store each value in a minimum distance matrix $M$. The position in the matrix corresponds to the trajectory ID. So the value at $M(\tau_i, \tau_j)$ would be the minimum distance from trajectory $\tau_i$ to trajectory $\tau_j$. Itӳ good to note that this value will be the same as $M(\tau_j,\tau_i)$. Furthermore $M(\tau_i,\tau_i)$ will always be zero. These facts can be used to reduce computation while filling out the minimum distance matrix.
...
w_1 = \left\{
	\begin{array}{l l}
		u_2 / (u_1 + u_2)	&	\quad \text{if the point wonӴ go outside the patch.}\\
